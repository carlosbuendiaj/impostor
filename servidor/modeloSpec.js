...
describe("El juego del impostor", function() {
  ...
  it("comprobar valores de la partida",function(){
	...
	it("Ana se une y selecciona la skin", function(){
			juego.unirAPartida(codigo,"ana");
			var num=Object.keys(juego.partidas[codigo].usuarios).length;
	  		expect(num).toEqual(2);
	  		expect(juego.partidas[codigo].fase.nombre).toEqual("inicial");
	  		var partida=juego.partidas[codigo];
	  		partida.usuarios["ana"].skinSeleccionado(1);
	  		expect(partida.usuarios["ana"].idSkin).toEqual(1);
		});
	it("Owner selecciona la skin", function(){
	  		expect(juego.partidas[codigo].fase.nombre).toEqual("inicial");
	  		var partida=juego.partidas[codigo];
	  		partida.usuarios[nick].skinSeleccionado(1);
	  		expect(partida.usuarios[nick].idSkin).toEqual(1);
		});
   
   	it("Ana se une y no selecciona skin", function(){
			juego.unirAPartida(codigo,"ana");
			var num=Object.keys(juego.partidas[codigo].usuarios).length;
	  		expect(num).toEqual(2);
	  		expect(juego.partidas[codigo].fase.nombre).toEqual("inicial");
	  		var partida=juego.partidas[codigo];
	  		partida.usuarios["ana"].skinSeleccionado("undefined");
	  		expect(partida.usuarios["ana"].idSkin).toEqual("undefined");
		});

   	it("Owner no selecciona la skin", function(){
	  		expect(juego.partidas[codigo].fase.nombre).toEqual("inicial");
	  		var partida=juego.partidas[codigo];
	  		partida.usuarios[nick].skinSeleccionado("undefined");
	  		expect(partida.usuarios[nick].idSkin).toEqual("undefined");
		});
