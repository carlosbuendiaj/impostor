...
function lanzarJuego(){
  ...
  var skins=[{frame:0, sprite:"chrono"}]
  var remotos;
  var muertos;
  var capaTareas;
  var tareasOn=true;
  var trampillaOn=true;
  var ataquesOn=true;
  var final=false;
  ...
  function preload() {
    ...
    this.load.spritesheet("chrono","cliente/assets/images/chronoWalking.png",{frameWidth:46,frameHeight:72});
  ...
  function create() {
    ...
    const anims4 = crear.anims;
      ...
      anims6.create({
        key: "chrono-left-walk",
        frames: anims.generateFrameNames("chrono", {
          //prefix: "misa-left-walk.",
          start: 5,
          end: 8,
          //zeroPad: 3
...
function Tp(sprite, obj){
    //comprobar si el jugador, es un impostor, y si el objeto.properties.trampilla
    console.log("entra en la funcion");
    if(ws.impostor){
      console.log("sabe que es importor  " +ws.impostor);
      console.log(obj.properties.trampilla);
      if (obj.properties.trampilla =="trampilla") {
        console.log("ve que el objeto tiene la propiedad trampilla" + obj.properties.trampilla );
        if (teclaT.isDown) {
           console.log("AAAAAAAAAAAAAHHHHHHHHHHHH INTERACTUA");
      trampillaOn=false;
        }
      }
     
        //teletransportar al jugador a cualquiera de las otras trampilla. Quiza la selecciona él?
        //jugadores[impostor].move();
    //}
    //trampilla=
  }
}
  ...
  function lanzarJugador(nick,numJugador){
    ...
    //Seleccionar Skin en la linea de abajo, dependiendo del click del jugador.
    if(ws.idSkin == "1"){
    player = crear.physics.add.sprite(x, spawnPoint.y,"chrono",skins[ws.idSkin - 1].frame); 
    }
    else{
    ...
    crear.physics.add.collider(player, capaTareas, Tp ,()=>{return trampillaOn});
    }
  ...
  function lanzarJugadorRemoto(nick,numJugador){
    ...
    var x=spawnPoint.x+numJugador*24+2;
    if(ws.idSkin =="1"){
      jugadores[nick]=crear.physics.add.sprite(x, spawnPoint.y,"chrono",skins[ws.idSkin - 1].frame);   
    }else{
  ...
  function mover(datos)
  {
    ...
    if (remoto && !final)
    {
      ...
      if (ws.idSkin == undefined) {
        ...
        var skinaux = skins[ws.idSkin - 1].sprite;
        if (direccion=="left") {
          remoto.anims.play(skinaux+"-left-walk", true);
        } else if (direccion=="right") {
          remoto.anims.play(skinaux+"-right-walk", true);
        } else if (direccion=="up") {
          remoto.anims.play(skinaux+"-back-walk", true);
        } else if (direccion=="down") {
          remoto.anims.play(skinaux+"-front-walk", true);
        } else {
          remoto.anims.stop();
        }
      }
  ...
  */
  if (ws.idSkin == undefined) {
        ...
        var skinaux = skins[ws.idSkin - 1].sprite;
        if (direccion=="left") {
          player.anims.play(skinaux+"-left-walk", true);
        } else if (direccion=="right") {
          player.anims.play(skinaux+"-right-walk", true);
        } else if (direccion=="up") {
          player.anims.play(skinaux+"-back-walk", true);
        } else if (direccion=="down") {
          player.anims.play(skinaux+"-front-walk", true);
        } else {
          player.anims.stop();
        }
      }
